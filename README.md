...
- После выбора направления передает запрос ChatGPT и получает готовый перевод.
- Результат отправляется пользователю с возможностью:
  - Перейти в /translator для выбора направления для перевода.
  - Закончить.
---
...
- При обработке команды отправляет заранее заготовленное изображение.
- Предлагает выбор из нескольких разедлов с помощью кнопок.
- После выбора предлагает выбор из нескольких жанров с помощью кнопок.
- После выбора раздела и жанра передает запрос ChatGPT и получает рекомендацию.
- Результат отправляется пользователю с возможностью:
  - Не нравится.
  - Закончить.
- Бот запомиает список ненравится, но только за сессию.
- 
## Установка и запуск
